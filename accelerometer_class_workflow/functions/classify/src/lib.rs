...
impl EdgeFunction for ClassifyFun {
    ...
    fn handle_cast(_src: InstanceId, encoded_message: &[u8]) {
        log::info!("classify: handle_cast() called");
        ...
        log::info!("classify: classified the received features");

        let serialized_classification_result = match serde_json::to_string(&classification_result) {
            Ok(json) => json,
            Err(e) => {
                eprintln!("Error serializing classification result: {}", e);
                String::new()
            }
        };
 ...
 fn handle_init(_payload: Option<&[u8]>, _init_state: Option<&[u8]>) {
        ...
        log::info!("CLASSIFY: started");
    }

    fn handle_stop() {
        log::info!("CLASSIFY: stopped");
    }
}
