...
impl EdgeFunction for ExtractFeaturesFun {
    ...
    fn handle_cast(_src: InstanceId, encoded_message: &[u8]) {
        log::info!("extract_features: handle_cast() called");
        ...
        log::info!("extract_features: features have been extracted");

        let serialized_features = match serde_json::to_string(&features) {
            Ok(json) => json,
            Err(e) => {
                eprintln!("Error serializing extracted features: {}", e);
                String::new()
            }
        };
    ...
    fn handle_call(_src: InstanceId, _encoded_message: &[u8]) -> CallRet {
        log::info!("extract_features: handle_call() called");
        CallRet::NoReply
    }

    fn handle_init(_payload: Option<&[u8]>, _init_state: Option<&[u8]>) {
        edgeless_function::init_logger();
        log::info!("EXTRACT_FEATURES: started");

    }

    fn handle_stop() {
        log::info!("EXTRACT_FEATURES: stopped");
    }
}
